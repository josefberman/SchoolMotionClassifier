...
def train_classifier(
    ...
    le = LabelEncoder()
    # Fit on all canonical labels so classes are stable
    le.fit(list(CANONICAL))
    ...
    if len(X_test):
        ...
        report["sim_test_macro_f1"] = float(f1_score(y_test, pred, average="macro", labels=list(CANONICAL), zero_division=0))
        report["classification_report"] = classification_report(
            y_test, pred, labels=list(CANONICAL), zero_division=0, output_dict=True
        )
        cm = confusion_matrix(y_test, pred, labels=list(CANONICAL))
        report["confusion_matrix"] = cm.tolist()
        report["confusion_labels"] = list(CANONICAL)
